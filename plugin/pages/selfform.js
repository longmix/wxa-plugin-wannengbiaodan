Page({
    ...
    onLoad: function (options) {
      ...
      //====  这三项参数因为是在插件内部的Page中，所以只能通过这个page的参数传入进来 ======
      //如果不想显示传递这三个参数，请是组件模式调用
      if(options.openid){
      ...
      selfform_data.get_selfform_data(selfform_data_params);
    ...
    __selfform_data_callback:function(callback_data){
      ...
      //console.log('selfform_data_callback====>>>>', callback_data);
    ...
    link_item_click: function(e){
      ...
      //这里不能直接使用这个函数，因为这个函数里面有 h5browser（页面内容为webview），
      //而小程序插件禁止使用webview
      ...
      console.log('aaaaaaa1111===>>>'+miniProgramExports);

      if(!miniProgramExports){
        return;
      }

      console.log('aaaaaaa2222===>>>'+typeof(miniProgramExports.greeting));

      if(typeof(miniProgramExports.greeting) == "function"){
        var str001 = miniProgramExports.greeting('abcdefg'); // 'Greetings from Weixin MiniProgram!'
        console.log('aaaaaaa===>>>'+str001);
      }

      if(typeof(miniProgramExports.link_item_click) == "function"){
        miniProgramExports.link_item_click(url);
      }
    ...
    copy_text: function(e){
      ...
      if(typeof(miniProgramExports.copy_text) == "function"){
        miniProgramExports.copy_text(text);
      }
